...
export default function MobileDashboardPage() {
  ...
  useEffect(() => {
    if (!session) return;
    const channel = supabase.channel("mobile-dashboard-realtime")
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "notes" }, async () => { await fetchStats(); await fetchLeaderboard(); })
      .on("postgres_changes", { event: "UPDATE", schema: "public", table: "notes" }, async () => { await fetchStats(); await fetchLeaderboard(); })
      .on("postgres_changes", { event: "DELETE", schema: "public", table: "notes" }, async () => { await fetchStats(); await fetchLeaderboard(); })
      .on("postgres_changes", { event: "UPDATE", schema: "public", table: "profiles" }, async () => { await fetchStats(); await fetchLeaderboard(); })
      .subscribe();
  ...
  async function confirmDelete() {
    ...
    try {
      ...
      if (fileName) await supabase.storage.from("materials").remove([fileName]);
    ...
    <div className="min-h-screen transition-all duration-500" style={{background: bg, color: textColor}}>
      <div className="p-4 pb-24">
        ...
        <div className="flex items-center justify-between mb-6 pt-4">
          <div>
            <p className="text-xs font-medium tracking-widest uppercase mb-1" style={{color: subTextColor}}>Dashboard</p>
            <h1 className="text-2xl font-bold">Hi, <span className="glow-text">{fullName}</span> 👋</h1>
            <div className="mt-1 h-0.5 w-12 rounded-full" style={{background: "linear-gradient(90deg, #8b0000, transparent)"}}></div>
          </div>
          <button
        ...
        <div className="grid grid-cols-3 gap-2 mb-6">
          {[
            ...
            <div key={stat.label} className="shine-effect p-3 rounded-2xl text-center" style={{background: cardBg}}>
              <p className="text-lg">{stat.icon}</p>
              <p className="text-xl font-bold">{stat.value}</p>
              <p className="text-xs" style={{color: subTextColor}}>{stat.label}</p>
            </div>
          ))}
        </div>

        {/* Leaderboard */}
        <div className="shine-effect p-4 rounded-2xl mb-6" style={{background: cardBg}}>
          ...
          <div className="space-y-2">
            {leaderboard.map((user, index) => (
              <div key={index} className="flex items-center justify-between p-2 rounded-xl" style={{background: noteBg}}>
                <div className="flex items-center gap-2">
                  <span className="text-sm font-bold" style={{color: index === 0 ? "#FFD700" : index === 1 ? "#C0C0C0" : index === 2 ? "#CD7F32" : "#888"}}>#{index + 1}</span>
                  <span className="text-sm font-medium truncate max-w-30">{user.full_name}</span>
                </div>
                <span className="text-xs font-bold" style={{color: "#ff6666"}}>{user.xp} XP</span>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* My Notes */}
        <h2 className="text-xl font-bold mb-3">My Notes</h2>
        <div className="space-y-2">
          {subjects.map((subject) => {
            ...
            return (
              <div key={subject} className="rounded-2xl overflow-hidden" style={{background: cardBg2, border}}>
                <button
                ...
                >
                  ...
                  <span className="text-xs" style={{color: subTextColor}}>{subjectNotes.length} {isOpen ? "▲" : "▼"}</span>
                </button>
                {isOpen && (
                  <div className="px-3 pb-3 space-y-2">
                    {subjectNotes.map((note) => (
                      <div key={note.id} className="p-3 rounded-xl flex items-center justify-between gap-2" style={{background: noteBg}}>
                        <div className="flex-1 min-w-0">
                          <p className="font-bold text-xs truncate">{note.title}</p>
                          <p className="text-xs mt-0.5" style={{color: subTextColor}}>❤️ {note.likes || 0}</p>
                        </div>
                        <div className="flex gap-1 shrink-0">
                          <a href={note.file_url} target="_blank" className="px-2 py-1 rounded-lg text-xs" style={{background: darkMode ? "#1e3a5f" : "#dbeafe", color: darkMode ? "#fff" : "#1e3a5f"}}>Open</a>
                          <button onClick={() => setDeleteTarget(note)} className="bg-red-600 px-2 py-1 rounded-lg text-xs text-white">Del</button>
                        </div>
                      </div>
                    ))}
                  </div>
      ...
      {deleteTarget && (
        <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 p-4">
          <div className="p-6 rounded-3xl w-full max-w-xs text-center" style={{background: darkMode ? "#1a0000" : "#fff5f5", color: textColor}}>
            <p className="text-4xl mb-3">🗑️</p>
            <h2 className="text-lg font-bold mb-2">Delete Note?</h2>
            <p className="mb-1 text-xs" style={{color: subTextColor}}>This will be deleted permanently</p>
            <p className="text-xs mb-2" style={{color: subTextColor}}>20XP will be deducted</p>
            <p className="font-bold mb-4 text-xs">"{deleteTarget.title}"</p>
            <div className="flex gap-3">
              <button onClick={() => setDeleteTarget(null)} className="flex-1 p-2 rounded-xl font-bold text-sm" style={{background: darkMode ? "#3f3f3f" : "#e5e5e5", color: textColor}}>Cancel</button>
              <button onClick={confirmDelete} className="flex-1 bg-red-600 p-2 rounded-xl font-bold text-white text-sm">Delete</button>
            </div>
          </div>
        </div>
      )}
