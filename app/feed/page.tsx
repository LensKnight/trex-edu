...
export default function FeedPage() {
  ...
  async function fetchNotes() {
    ...
    if (!profile) return;
    const { data } = await supabase.from("notes").select("*").eq("class_name", profile.class_name).eq("section", profile.section).order("created_at", { ascending: false });
  ...
  async function likeNote(note: Note) {
    ...
    if (error) { setLiking(null); return alert("Like failed"); }
  ...
  return (
    <div className="p-4 md:p-8 min-h-screen transition-all duration-500" style={{background: bg, color: textColor}}>
      ...
      <div className="flex items-start justify-between mb-6 md:mb-10">
        <div>
          <p className="text-xs md:text-sm font-medium tracking-widest uppercase mb-1" style={{color: subTextColor}}>Community Notes</p>
          <h1 className="text-3xl md:text-5xl font-bold">Notes Feed</h1>
      ...
      {subjects.map((subject) => {
        ...
        return (
          <div key={subject} className="mb-8 md:mb-12">
            <h2 className="text-2xl md:text-3xl font-bold mb-3 md:mb-5">{subject}</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-5">
              {filteredNotes.map((note) => {
                const alreadyLiked = likedNotes.includes(note.id);
                const isOwn = note.uploader_id === session?.user.id;
                return (
                  <div key={note.id} className="p-4 md:p-5 rounded-3xl transition-all duration-300" style={{background: cardBg, border}}>
                    <h3 className="text-lg md:text-2xl font-bold mb-2 truncate">{note.title}</h3>
                    <p className="mb-3 md:mb-4 text-xs md:text-sm" style={{color: subTextColor}}>{note.subject}</p>
      ...
      {filteredBySearch.length === 0 && search && (
        <div className="text-center mt-20">
          ...
          <p className="text-lg font-bold" style={{color: subTextColor}}>No notes found for "{search}"</p>
        </div>
      )}
